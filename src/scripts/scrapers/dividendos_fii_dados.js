...
async function salvarDividendos(fiiID, dividendos) {
    ...
    try {
        ...
        for (const item of dividendos) {
            await client.query(insertQuery, [fiiID, item.DataPagamento, item.ValorPagamento]);
        }

        console.log(`Dividendos do fundo com FundoImobiliarioID ${fiiID} inseridos/atualizados com sucesso!`);
    } finally {
        client.release();
    }
}
...
async function executar() {
    try {
        ...
        if (fundos.length === 0) {
            console.log("Nenhum Fundo Imobiliário encontrado na base de dados para processar.");
            return;
        }
