    ...
    <div v-else class="conteudo">
        ...
        <div v-if="showGraph" class="chart-section">
            ...
            <div class="chart-container">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 300" class="chart-svg">
                    <!-- Grid lines -->
                    <defs>
                        <pattern id="grid" width="40" height="20" patternUnits="userSpaceOnUse">
                            <path d="M 40 0 L 0 0 0 20" fill="none" stroke="rgba(255,255,255,0.05)" stroke-width="1"/>
                        </pattern>
                    </defs>
                    <rect width="100%" height="100%" fill="url(#grid)" />
                    <!-- Axes -->
                    <line x1="50" y1="250" x2="750" y2="250" stroke="#94a3b8" stroke-width="2"/>
                    <line x1="50" y1="50" x2="50" y2="250" stroke="#94a3b8" stroke-width="2"/>
                    <!-- Bars -->
                    <g v-for="(item, index) in dadosPorMes" :key="item.mes">
                        <text :x="50 + index * groupWidth + groupWidth/2" y="270" text-anchor="middle" font-size="12" fill="#f8fafc">Mês {{ item.mes }}</text>
                        <rect v-if="item.acao"
                ...
                <div class="detail-card">
                    ...
                    <div class="scroll-list">
                        <div v-for="acao in dadosAcoes" :key="acao.Ticker" class="item">
                            {{ acao.Ticker }} - {{ acao.Descricao }} | R$ {{ (acao.ValorPagamento * acao.Quantidade).toFixed(2) }} | {{ new Date(acao.DataPagamento).toISOString().slice(0,10) }}
                        </div>
                    </div>
                </div>
                <div class="detail-card">
                    ...
                    <div class="scroll-list">
                        <div v-for="fii in dadosFii" :key="fii.Ticker" class="item">
                            {{ fii.Ticker }} - {{ fii.Descricao }} | R$ {{ (fii.ValorPagamento * fii.Quantidade).toFixed(2) }} | {{ new Date(fii.DataPagamento).toISOString().slice(0,10) }}
                        </div>
                    </div>
                </div>
            </div>
